...
class ExportCommand extends Command {
  async run() {
    ...
    shell.exec('docker-compose --project-directory ' + project.getProjectPath() + ' run --rm --no-deps wordpress-cli wordup export ' + project.getWordupPkgB64() + ' --type=' + exportType+exportParams, function (code, stdout, stderr) {
      if(code !== 0){
        return;
      }
